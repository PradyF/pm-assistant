...
def main():
    ...
    for title, col, resp, mem, due in [
        ("Настроить API интеграции с 1С", "В работе", "Саша", ["Руководитель"], -5),
        ("Согласовать ТЗ с заказчиком", "Ревью", "Маша", ["Руководитель", "Иван"], -3),
        ("Подготовить отчёт за август", "В работе", "Иван", ["Руководитель"], -2),
    ]:
        create_card(title, col, resp, mem, iso(due))

    # В работе (4)
    for title, col, resp, mem, due in [
        ("Разработать личный кабинет пользователя", "В работе", "Маша", ["Саша", "Руководитель"], 7),
    ...
    # Ревью (5)
    for title, col, resp, mem, due in [
        ("Проверить код модуля авторизации", "Ревью", "Саша", ["Руководитель"], 2),
        ("Тестирование формы регистрации", "Ревью", "Маша", ["Саша"], 3),
        ("Проверить интеграцию с платёжной системой", "Ревью", "Иван", ["Руководитель"], 1),
        ("Согласовать UX главной страницы", "Ревью", "Руководитель", ["Маша", "Саша"], 4),
    ...
    for title, col, resp, mem, due in [
        ("Настроить SSL сертификат", "Готово", "Саша", ["Руководитель"], 1),
        ("Обновить главную страницу", "Готово", "Маша", ["Руководитель"], 2),
        ("Исправить баг в корзине", "Готово", "Иван", ["Саша"], 1),
        ("Развернуть тестовый стенд", "Готово", "Руководитель", ["Иван"], 3),
        ("Подготовить отчёт за квартал", "Готово", "Руководитель", ["Маша"], 2),
    ]:
        create_card(title, col, resp, mem, iso(due))
